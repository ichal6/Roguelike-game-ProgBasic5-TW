...
def ask_untill_correct(list_label, end_range):
    ...
    while is_correct is not True:
        id_of_record = terminal_view.get_inputs(list_label, "Please enter value ")
        is_correct = common.check_is_number(id_of_record[FIRST_ELEMENT_IN_LIST], end_range)
...
def run():
    ...
    while choice != "0":
        ...
        if choice == "1":
            ...
            id_of_record_to_remove = ask_untill_correct(["Please insert ID"], len(list_of_accounting))
            ...
            id_of_record_to_update = ask_untill_correct(["Please insert ID"], len(list_of_accounting))
            ...
            year_of_check = ask_untill_correct(["Year of publishing: "], MAX_YEAR)
